...
with dai.Device(pipeline) as device:
    ...
    while True:
        ...
        if depth is not None:
            depth = depth.getFrame()
            depth = cv2.normalize(depth, None, 255, 0, cv2.NORM_INF, cv2.CV_8UC1)
            depth = cv2.applyColorMap(depth, cv2.COLORMAP_HoT)
            for detection in detections:
                bbox = frameNorm(croppedFrame,(detection.xmin, detection.ymin, detection.xmax, detection.ymax))
                bbox[::2] += offsetX
                cv2.rectangle(depth, (bbox[0],bbox[1]),(bbox[2],bbox[3]),(255,0,0),2)
                cv2.putText(depth, labelMap[detection.label], (bbox[0]+10, bbox[1]+20), cv2.FONT_HERSHEY_TRIPLEX, .5, 255)
                cv2.putText(depth, f"/int(detection.confidence*100)/%", (bbox[0]+10, bbox[1]+40), cv2.FONT_HERSHEY_TRIPLEX, .5, 255)
